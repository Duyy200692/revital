      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-8 gap-y-16">
        {isAdmin && (
          ...
          >
            ...
            <span className="mt-4 font-brand font-black text-[10px] uppercase tracking-widest text-gray-400 group-hover:text-primary">Thêm Món Mới</span>
          </div>
        )}
        ...
        {filteredProducts.map((product) => (
          <div key={product.id} className="group relative flex flex-col overflow-hidden rounded-[2.5rem] aspect-[4/5] shadow-sm hover:shadow-2xl transition-all duration-500 bg-dark">
            ...
            {/* Darker Gradient Overlay for clear text */}
            ...
            <div className="absolute inset-0 p-8 flex flex-col justify-end pointer-events-none">
              <div className="space-y-3 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                ...
                <div className="pt-2 flex items-center gap-2 text-white font-brand font-black text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity pointer-events-auto">
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl p-8 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
            ...
            <form className="space-y-4" onSubmit={handleAddSubmit}>
              <div>
                ...
                <input required className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={newProduct.name} onChange={(e) => setNewProduct({...newProduct, name: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Giá</label>
                  <input required className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={newProduct.price} onChange={(e) => setNewProduct({...newProduct, price: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Hình ảnh (URL)</label>
                  <input required className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={newProduct.image} onChange={(e) => setNewProduct({...newProduct, image: e.target.value})} />
                </div>
              </div>
              <button type="submit" className="w-full py-4 bg-primary text-white font-brand font-black uppercase rounded-2xl">Thêm Vào Menu</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[400] bg-dark/80 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl p-8 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setEditingId(null); }}>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Tên món</label>
                <input className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={products.find(p => p.id === editingId)?.name || ''} onChange={(e) => updateProduct(editingId, { name: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Giá</label>
                  <input className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={products.find(p => p.id === editingId)?.price || ''} onChange={(e) => updateProduct(editingId, { price: e.target.value })} />
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Hình ảnh (URL)</label>
                  <input className="w-full bg-accent border-none rounded-2xl px-5 py-3 text-sm" value={products.find(p => p.id === editingId)?.image || ''} onChange={(e) => updateProduct(editingId, { image: e.target.value })} />
                </div>
              </div>
              <button type="submit" className="w-full py-4 bg-primary text-white font-brand font-black uppercase rounded-2xl">Lưu Thay Đổi</button>
            </form>
          </div>
        </div>
      )}
