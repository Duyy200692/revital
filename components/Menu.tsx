...
const Menu: React.FC = () => {
  ...
  return (
    <div className="max-w-[1920px] mx-auto px-6 lg:px-10 xl:px-24">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-x-8 gap-y-16">
        ...
        {filteredProducts.map((product) => (
          <div key={product.id} className="group flex flex-col relative">
            ...
            <div className="relative w-full aspect-[4/5] overflow-hidden rounded-[2.5rem] bg-accent mb-6 shadow-sm group-hover:shadow-2xl transition-all duration-500">
              ...
              {product.popular && (
                <div className="absolute top-6 left-6 bg-primary text-white text-[10px] font-brand font-black uppercase tracking-widest px-4 py-1.5 rounded-full z-10">Best Seller</div>
              )}
            </div>
            
            <div className="space-y-2 px-2">
              <div className="flex justify-between items-start gap-2">
                <h4 className="text-lg font-brand font-bold text-dark leading-tight">{product.name}</h4>
                <span className="text-sm font-black text-primary">{product.price}</span>
              </div>
              <p className="text-gray-400 text-xs font-sans leading-relaxed line-clamp-2">{product.description}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* EDIT MODAL (Giữ nguyên nhưng cập nhật logic popular) */}
      {editingId && (
        <div className="fixed inset-0 z-[400] bg-dark/80 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl p-8 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
            <div className="flex justify-between items-center mb-6">
