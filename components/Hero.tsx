    ...
    <div className="relative h-[600px] lg:h-[900px] w-full overflow-hidden bg-accent">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-24 w-full">
              <div className={`space-y-4 lg:space-y-6 max-w-2xl transition-all duration-1000 delay-300 ${currentIndex === idx ? 'translate-x-0 opacity-100' : '-translate-x-20 opacity-0'}`}>
                <div className="space-y-2">
                  <span className="inline-block bg-primary text-white text-[10px] lg:text-xs font-brand font-black uppercase tracking-[0.4em] px-4 py-2 rounded-lg">{campaign.subtitle}</span>
                ...
                <p className="text-gray-300 text-base lg:text-xl font-sans leading-relaxed max-w-xl opacity-90">
                  {campaign.description}
                </p>
                <div className="flex items-center gap-6 pt-4">
                  <button className="px-8 lg:px-14 py-4 lg:py-5 bg-primary text-white font-brand font-black uppercase tracking-widest text-xs lg:text-sm rounded-full shadow-2xl shadow-primary/40 hover:scale-105 transition-all">
                    {campaign.ctaText}
                  </button>
      ...
      <div className="absolute bottom-12 left-0 w-full z-40 px-6 lg:px-24 flex flex-row items-center justify-between pointer-events-none">
        <div className="flex gap-4 pointer-events-auto">
          <button onClick={prevSlide} className="w-12 h-12 lg:w-16 lg:h-16 rounded-full bg-white/10 backdrop-blur-md border border-white/20 shadow-xl flex items-center justify-center text-white hover:bg-primary transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 lg:h-7 lg:w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
          </button>
          <button onClick={nextSlide} className="w-12 h-12 lg:w-16 lg:h-16 rounded-full bg-white/10 backdrop-blur-md border border-white/20 shadow-xl flex items-center justify-center text-white hover:bg-primary transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 lg:h-7 lg:w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </button>
        </div>
        <div className="bg-white/10 backdrop-blur-md border border-white/20 px-6 lg:px-8 py-3 lg:py-4 rounded-full flex items-center gap-4 shadow-2xl pointer-events-auto">
          ...
          <div className="flex gap-2 h-1.5 items-center">
            {campaigns.map((_, idx) => (
              <div key={idx} className={`h-full rounded-full transition-all duration-700 ${currentIndex === idx ? 'w-12 lg:w-16 bg-primary' : 'w-3 lg:w-4 bg-white/20'}`}></div>
            ))}
          </div>
